...
class RnnPredictor:
    ...
    def predict(self, country_code: str, horizon: int = 9, env: str = 'cloud') -> bool:
        ...
        df: DataFrame = pd.read_csv(self.get_preprocessed_data_path_no_gdp)

        target = 'CO2'
        test_country = country_code

        columns = [c for c in df.columns if c.endswith(test_country)]
        ...
        actual = df[f'{target}_{test_country}'].values * std + mean
